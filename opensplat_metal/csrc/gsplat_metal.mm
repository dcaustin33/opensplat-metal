...
MetalContext* init_gsplat_metal_context() {
    ...
    // Use dladdr to find the directory of this shared library
    Dl_info dl_info;
    NSString *metallib_path = nil;
    if (dladdr((void*)init_gsplat_metal_context, &dl_info) && dl_info.dli_fname) {
        NSString *so_path = [NSString stringWithUTF8String:dl_info.dli_fname];
        NSString *so_dir = [so_path stringByDeletingLastPathComponent];
        metallib_path = [so_dir stringByAppendingPathComponent:@"default.metallib"];
    }

    if (metallib_path != nil && [[NSFileManager defaultManager] fileExistsAtPath:metallib_path]) {
        NSURL *libURL = [NSURL fileURLWithPath:metallib_path];
        printf("%s: loading '%s'\n", __func__, [metallib_path UTF8String]);
